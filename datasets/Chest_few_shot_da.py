...
class CustomDatasetFromImages(Dataset):
    def __init__(self, csv_path=ChestX_path+"/Data_Entry_2017.csv", \
        ...
        for name, label in zip(self.image_name_all,self.labels_all):
            ...
            if len(label) == 1 and label[0] != "No Finding" and label[0] != "Pneumonia" and label[0] in self.used_labels:
                ...
                self.label_names.append(label[0])
    ...
    def __getitem__(self, index):
        ...
        single_label_name = self.label_names[index]

        #print("zzzzzzzzzzzz")
        #print(single_image_label)
        #print(single_label_name)

        return (single_label_name, img_as_img, single_image_label)
...
class SetDataset:
    def __init__(self, batch_size, transform):
        ...
        for i, (text, data, label) in enumerate(d):
            ...
            self.sub_meta[label].append((text, data))
...
class SubDataset:
    ...
    def __getitem__(self,i):
        ...
        for transform in self.transform:
            #img.append(transform(self.sub_meta[i]))
            #target.append(self.target_transform(self.cl))
            img.append(transform(self.sub_meta[i][1]))
            target.append(self.target_transform(self.cl))
            text.append(self.sub_meta[i][0])
    ...
    def get_composed_transform(self, aug=False):
        if aug:
            transform_list = [['Resize', 'ToTensor', 'Normalize'],
                              ['Resize', 'ImageJitter', 'RandomHorizontalFlip', 'ToTensor', 'Normalize'],
                              ['RandomResizedCrop', 'ToTensor', 'Normalize'],
                              ['RandomResizedCrop', 'ImageJitter', 'ToTensor', 'Normalize'],
