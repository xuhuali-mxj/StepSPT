...
class CustomDatasetFromImages(Dataset):
    def __init__(self, csv_path= ISIC_path + "/ISIC2018_Task3_Training_GroundTruth/ISIC2018_Task3_Training_GroundTruth.csv", \
        ...
        # Read the csv file
        self.name = list(pd.read_csv(csv_path).columns[1:])
        self.data_info = pd.read_csv(csv_path, skiprows=[0], header=None)

        # First column contains the image paths
        self.image_name = np.asarray(self.data_info.iloc[:, 0])

        self.alabels = np.asarray(self.data_info.iloc[:, 1:])
        self.labels = (self.alabels!=0).argmax(axis=1)
    ...
    def __getitem__(self, index):
        ...
        single_label_name = self.name_list
...
class SetDataset:
    def __init__(self, batch_size, transform):
        ...
        for i, (text, data, label) in enumerate(d):
            ...
            self.sub_meta[label].append((text[i], data))
